...
class LinearRegression2Features3D(ThreeDSlide):
    def construct(self):
        ...
        # **CORRECTED:** Use `always_redraw` to create the entire VGroup each frame.
        eq3d = always_redraw(
            lambda: VGroup(
                MathTex(r"\\hat{y} =").scale(0.8),
                DecimalNumber(w1_tracker.get_value(), num_decimal_places=2).scale(0.8),
                MathTex(r" x_1 +").scale(0.8),
                DecimalNumber(w2_tracker.get_value(), num_decimal_places=2).scale(0.8),
                MathTex(r" x_2 +").scale(0.8),
        ...
        mse_group = always_redraw(
            lambda: VGroup(
                MathTex("MSE:").scale(0.7),
                DecimalNumber(0, num_decimal_places=3).scale(0.7)
            )
            .arrange(RIGHT, buff=0.05)
            .next_to(eq3d, DOWN, buff=0.15, aligned_edge=RIGHT)
        )
        # **CORRECTED:** The MSE updater must be outside the lambda,
        # or the value will reset every frame. A better solution is to
        # update it directly inside the lambda, which simplifies things.
        # This re-writes the lambda for mse_group to include the calculation.
        mse_group = always_redraw(
            lambda: VGroup(
                MathTex("MSE:").scale(0.7),
                DecimalNumber(
                    0.5 * float(np.mean(
                        ( (w1_tracker.get_value() * x1 + w2_tracker.get_value() * x2_target + b_tracker.get_value()) - y_3d )**2
                    ))
                    , num_decimal_places=3).scale(0.7)
            )
            .arrange(RIGHT, buff=0.05)
            .next_to(eq3d, DOWN, buff=0.15, aligned_edge=RIGHT)
        )
        ...
        self.play(
            *[p.animate.move_to(axes3d.c2p(x1[i], x2_target[i], y_3d[i])) for i, p in enumerate(points3d)],
            run_time=1.2,
            lag_ratio=0.02
        )
